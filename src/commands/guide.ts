...
export const GUIDE_TOPICS: ReadonlyArray<readonly [topic: string, summary: string]> = [
  ["service", "first Service setup, inspect, search, read, and update path"],
...
export function executeGuideCommand(args: readonly string[]): GuideCommandResult {
  ...
  try {
    ...
    if (subtopic) {
      throw new KitUsageError(`unknown guide topic '${topic} ${subtopic}'. Available topics: service, service qmd, client, propose`);
    }
    switch (topic) {
      case "client":
      ...
      default:
        throw new KitUsageError(`unknown guide topic '${topic}'. Available topics: service, service qmd, client, propose`);
    }
  } catch (error) {
